...
function initializeColoringGame(panel) {
  ...
  Promise.all([
    ...
    product.coloringRegionsUrl
      ? fetch(assetUrl(product.coloringRegionsUrl)).then((response) => {
          if (!response.ok) throw new Error("Could not load coloring region data.");
          return response.json();
        })
      : Promise.resolve({ regions: coloringRegions, shapeGroups: coloringShapeGroups }),
    product.coloringRegionOverridesUrl
      ? fetch(assetUrl(product.coloringRegionOverridesUrl)).then((response) => {
          if (!response.ok) throw new Error("Could not load coloring region overrides.");
          return response.json();
        })
...
function addToCart() {
  ...
  if(found) {
    ...
    items.push({productId:product.id,variantId,qty,...(coloringDesign ? { coloringDesign } : {}),...(medalDetails || {})});
  }
